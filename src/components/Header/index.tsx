...
const convertidString = (str: string[]) => {
  let pageName: string = ""
  str.forEach((item) => {
    pageName += item.at(0)?.toUpperCase() + item.slice(1) + " "
  })
...
const Header = (props: {
  ...
  let pageName = pathName.split("/")[1].includes("-") ? pathName.split("/")[1].split("-").join(" "): pathName.split("/")[1];
  pageName = pageName.split(" ").length > 1 ? convertidString(pageName.split(" ")) : pageName.at(0)?.toUpperCase() + pageName.slice(1);
      ...
      <div className="flex flex-grow items-center justify-between px-4 py-5 shadow-2 md:px-5 2xl:px-10">
        ...
        <div className="hidden xl:block">
          <div>
            ...
            <p className="font-medium">Lista de Deudores</p>
          </div>
        </div>
