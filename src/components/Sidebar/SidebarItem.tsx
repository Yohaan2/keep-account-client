...
const SidebarItem = ({ item, pageName, setPageName, setSidebarOpen }: SidebarItemProps) => {
  ...
  const pageSegment = path.split("/")[1];
  return pageSegment.includes("-")
    ? pageSegment.replace(/-/g, " ").toLowerCase()
    : pageSegment.toLowerCase();
};

  const handleClick = useCallback(() => {
    const newPageName = item.label.toLowerCase();
    const currentPageName = getPageNameFromPath(pathName);
    const updatedPageName = newPageName !== pageName ? newPageName : currentPageName;
